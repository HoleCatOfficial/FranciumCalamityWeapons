...
{
    internal class DeuxièmeSwing : ModProjectile
    {

		SoundStyle DU_Swing = new SoundStyle("FranciumCalamityWeapons/Audio/DU_Swing") with {
				Volume = 1.0f, 
    			Pitch = 0.1f, 
    			PitchVariance = 0.5f, 
			};
        SoundStyle MissCrit = new SoundStyle("FranciumCalamityWeapons/Audio/MissedCrit");
    ...
    public override string Texture => "FranciumCalamityWeapons/Content/Projectiles/DeuxièmeSwing";
    ...
    private void PrepareStrike()
        ...
        if (Timer >= prepTime)
            {
            SoundEngine.PlaySound(DU_Swing); // Play sword sound here since playing it on spawn is too early
    ...
    private void ExecuteStrike()
        {
            if (CurrentAttack == AttackType.Swingdown)
                {
                Lighting.AddLight(Projectile.Center, new Color(147, 8, 79).ToVector3() * 1.0f);
            ...
            if (CurrentAttack == AttackType.Swingup)
                {
                SoundEngine.PlaySound(DU_Swing);
                Lighting.AddLight(Projectile.Center, new Color(147, 8, 79).ToVector3() * 1.0f);
                Progress = MathHelper.SmoothStep(0, -SWINGRANGE, (1f - UNWIND) * Timer / execTime);
    ...
    public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            if (hit.Crit == true)
            {
                ...
                Owner.GetModPlayer<ScreenshakePlayer>().screenshakeTimer = 24; // This is 1/10th of a second
            }
            else 
            {
                SoundEngine.PlaySound(MissCrit);
                // Create 'Miss!' text near the player
                Vector2 position = Owner.Center - new Vector2(0, 50); // Adjust position as needed
                CombatText.NewText(player.getRect(), Microsoft.Xna.Framework.Color.Yellow, "Miss!");
            }
        }
