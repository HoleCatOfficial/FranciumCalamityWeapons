...
namespace FranciumCalamityWeapons.Content.Projectiles
{
	...
	public class LesserDefiler_Flail : ModProjectile
	{
	
		public override string GlowTexture => "FranciumCalamityWeapons/Content/Projectiles/LesserDefilerFlailGlowmask";
		private const string ChainTexturePath = "FranciumCalamityWeapons/Content/Projectiles/NetherTendril"; // The folder path to the flail chain sprite
		...
		public override void SetDefaults() {
			...
			Projectile.height = 84; // The height of your projectile
		...
		public override void AI() {
            ...
            Projectile.rotation = Projectile.velocity.ToRotation() + MathHelper.PiOver2; // This is the default rotation code for flails, but it is overridden in PreDraw to make the flail rotate around the player.
			...
			float maxRetractSpeed = 50f; // The max speed the projectile will have while retracting
			...
			switch (CurrentAIState) {
				case AIState.Spinning: {
						...
						if (Projectile.owner == Main.myPlayer) {
							...
							if (!player.channel) // If the player releases then change to moving forward mode
							{
								...
								SoundEngine.PlaySound(new SoundStyle("FranciumCalamityWeapons/Audio/sn_swing_CalamityEntropy") with {PitchVariance = 1}, Projectile.position);
								SoundEngine.PlaySound(new SoundStyle("FranciumCalamityWeapons/Audio/MissedCrit") with {PitchVariance = 1}, Projectile.position);
								break;
							}
						}
				...
				case AIState.LaunchingForward: {
						...
						if (Locked == true && TargetNPC != null && TargetNPC.Distance(player.MountedCenter) > 100f)
						{
							TargetNPC.Center = Projectile.Center;
						}
						...
						if (shouldSwitchToRetracting) {
							...
							if (Locked == true && TargetNPC != null && TargetNPC.active == true)
							{
							SoundEngine.PlaySound(new SoundStyle("FranciumCalamityWeapons/Audio/ReelIn") with {PitchVariance = 1}, Projectile.position);
							}
			...
			if (freeRotation) {
				if (Projectile.velocity.Length() > 1f)
					Projectile.rotation = Projectile.velocity.ToRotation() + Projectile.velocity.X * 0.1f - MathHelper.PiOver2; // skid
				else
					Projectile.rotation += Projectile.velocity.X * 0.1f - MathHelper.PiOver2; // roll
			}
			else {
			...
			*/
			...
			if (Main.rand.NextBool(dustRate))
            {
				...
				//Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, ModContent.DustType<Dust>(), 0f, 0f, 150, default(Color), 1.3f);
		}
		...
		public override void ModifyHitNPC(NPC target, ref NPC.HitModifiers modifiers) {
			...
			// Flails do 100% more damage while launched or retracting. This is the damage the item tooltip for flails aim to match, as this is the most common mode of attack. This is why the item has ItemID.Sets.ToolTipDamageMultiplier[Type] = 2f;
				...
				Projectile.NewProjectile(Projectile.GetSource_OnHit(null), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<AnchorAreaParticle>(), 0, 0, Projectile.owner);
			}
	...
	struct Vertex
			{
				...
				public Vertex(Vector2 position, Vector3 textureCoordinates, Color color)
				{
					Position = position;
	...
	public class AnchorAreaParticle : ModProjectile
        {
            ...
            public override void AI()
            {
                ...
                for (int i = 0; i < dustAmount; i++)
                {
                    ...
                    Vector2 dustPos = Projectile.Center + radius * new Vector2((float)Math.Cos(angle), (float)Math.Sin(angle));
                    Dust dust = Dust.NewDustPerfect(dustPos, DustID.TintableDustLighted, null, 0, new Color(125, 191, 203), 1.0f);
                    dust.noGravity = true;
                    dust.fadeIn = 1.5f;
                }
