...
namespace FranciumCalamityWeapons.Content.Projectiles.StealthStrike
{
    ...
    public class ChromaStardust : ModProjectile
    {
        ...
        private void NormalAI()
        {
            ...
            {
                Dust dust = Dust.NewDustDirect(Projectile.position, Projectile.height, Projectile.width, DustID.BlueFairy, Projectile.velocity.X * .2f, Projectile.velocity.Y * .2f, 400, Scale: 1.2f);
                dust.velocity += Projectile.velocity * 0.3f;
                dust.velocity *= 0.2f;
            }
            if (Main.rand.NextBool(4))
            {
                Dust dust = Dust.NewDustDirect(Projectile.position, Projectile.height, Projectile.width, DustID.YellowStarDust, 0, 0, 254, Scale: 0.3f);
