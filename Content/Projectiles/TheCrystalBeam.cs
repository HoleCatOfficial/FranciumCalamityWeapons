...
{
	public class TheCrystalBeam : ModProjectile
	{
		// A helpful math constant for performing beam angling calculations.
		private const float PiBeamDivisor = MathHelper.Pi / TheCrystalHoldout.NumBeams;
		...
		private const float InnerBeamOpacityMultiplier = 1.0f;
		...
		private const float BeamColorHue = 0.0f;
		private const float BeamHueVariance = 1.0f;
		private const float BeamColorSaturation = 0.55f;
		private const float BeamColorLightness = 0.73f;
		...
		public override void AI() {
			...
			if (Projectile.type != ModContent.ProjectileType<TheCrystalBeam>() || !hostPrism.active || hostPrism.type != ModContent.ProjectileType<TheCrystalHoldout>()) {
				Projectile.Kill();
				return;
			}
			...
			float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / TheCrystalHoldout.MaxCharge, 0f, 1f);
			...
			Projectile.friendly = hostPrism.ai[0] > TheCrystalHoldout.DamageStart;

			// This offset is used to make each individual beam orient differently based on its Beam ID.
			float beamIdOffset = BeamID - TheCrystalHoldout.NumBeams / 2f + 0.5f;
			float beamSpread;
			float spinRate;
			...
			float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * TheCrystalHoldout.NumBeams) * MathHelper.TwoPi;
		...
		private Color GetOuterBeamColor() {
			...
			float hue = (BeamID / TheCrystalHoldout.NumBeams) % BeamHueVariance + BeamColorHue;
